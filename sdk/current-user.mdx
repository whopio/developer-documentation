...
const userToken = request.headers['x-whop-user-token'];
if (!userToken) return null;

const publicKey = createPublicKey(JWT_PEM);
const payload = jwt.verify(userToken, publicKey, {
  issuer: 'urn:whopcom:exp-proxy',
  algorithms: ['ES256']
});

const jwtAppId = payload.aud;
if (jwtAppId !== process.env.WHOP_APP_ID) return null;
...
- [Check a user's access](/sdk/check-access)