...
The main function exported from this package is the `createAppIframeSDK` function. When called, this function sets up a listener for messages form the main Whop site, using `window.on('message', ...)`.
...
We declare the SDK in a seperate file, such that we can import it in throughout the rest of the app.
