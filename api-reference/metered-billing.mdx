...
<Info>
  We are going to configured Metered Billing using the API while our UI is under
  construction.
</Info>

In this example, we will be charging a Seller for videos uploaded. It is very expensive for us, so we want to charge the Seller for howvever much they upload.

We are going to charge them $10 per every 5GB uploaded.

## 1. Retrieve your product ID

<Tip>Should we remove this example</Tip>
...
<Info>We are following the **structured** billing usage flow. A code snippet for variable billing usage is below</Info>
...
<Check>Congratulations. You have set up metered billing with Whop. You can now generate revenue.</Check>
