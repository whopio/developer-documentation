...
```bash
mkdir app/admin && mkdir "app/admin/[companyId]/[productId]" && touch "app/admin/[companyId]/[productId]/page.tsx"
```
...
3. Update the Admin Path in your [app configuration](https://dash.whop.com/settings/developer) to be `/admin/$companyId/$productId`
   ...
   <Frame caption="Update the admin path to be /admin/$companyId/$productId ">
     <img src="https://imgur.com/MpBp7Bl.png" />
   </Frame>
...
<Tip>
  We are going to skip error handling here because we just used this user's
  access token when retrieving the company. However, we recommend always
  handling errors. Handling the error will remove the `?`
</Tip>
