---
title: YouTube App
description: A tutorial on how to build a basic YouTube app. This app allows a seller to upload an unlisted video, and the user to play it directly on Whop
---

This tutorial demonstrates how to build a YouTube App. The app allows a seller to add a video link (even unlisted ones), and then the user to play it directly on Whop through an iFrame.
...
This will setup Prisma, now we need to tell it to generate our schema.
...
If you check `schema.prisma` you will see that Prisma has generated our schema for us.

To make use of our schema, we need to generate a client to enable us to interact with it. To do this, run the following command in your terminal:
...
Create a new path in your `app` folder:

```
app/seller/[companyId]/[productId]/index.tsx
```
...
Since our form will be a client component, we are going to create it in a separate file. Create a new file called `SellerForm.tsx` in your `components` folder, which should be in the root of your directory.
